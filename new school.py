...
file_analytics = open('textt.txt', 'a')
...
def menu():
    ...
    stat_button = Button(200, 425, 370, 50, (203, 190, 181), 0, 'Arial', 40, '      Статистика')
    while not gameover:
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = event.pos
                if x >= start_button.x and x <= start_button.x + start_button.width and y >= start_button.y and y <= start_button.y + start_button.height:
                    ...
                    exit()
                elif x >= stat_button.x and x <= stat_button.x + stat_button.width and y >= stat_button.y and y <= stat_button.y + stat_button.height:
                    analytics()
                    write_text(screen)
                ...
                if x >= start_button.x and x <= start_button.x + start_button.width and y >= start_button.y and y <= start_button.y + start_button.height:
                    ...
                    stat_button.border = 5
                    stat_button.process_draw(screen)
                else:
...
def write_text(screen):
    ...
    data = 'qqqqqqqqqqqqq'
    ts = font.render(data, True, black)
    screen.blit(ts, (0, 0))
...
def puzzle():
    ...
    while running:
        ...
        if snake[-1] == apple:
            print(length, apple)
            file_analytics.write(str(length) + ' ' + str(apple[0]) + ' ' + str(apple[1]) + '\n')
            apple = randrange(0, 800, cell), randrange(0, 700, cell)
            length += 1
            sound_eat.play()
